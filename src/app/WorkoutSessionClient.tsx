...
export function WorkoutSessionClient() {
    ...
    const [exercises, setExercises] = useState<{ id: string, name: string, sets: { weight: number, reps: number, done: boolean }[] }[]>(
        [{
            id: 'pullup',
            name: 'Pull - Up',
    ...
    useEffect(() => {
        ...
        function isSetItem(v: unknown): v is { weight: number, reps: number, done: boolean } {
            if (!isObject(v)) return false;
            return isNumber(v.weight) && isNumber(v.reps) && typeof v.done === 'boolean';
        }
        function isExercise(v: unknown): v is { id: string, name: string, sets: unknown[] } {
            if (!isObject(v)) return false;
        ...
        function isExerciseArray(v: unknown): v is { id: string, name: string, sets: { weight: number, reps: number, done: boolean }[] }[] {
    ...
    function addSet(exIdx: number) {
        setExercises(prev => prev.map((ex, i) => {
            ...
            return {
                ...ex,
                sets: [...ex.sets, { weight: ex.sets[ex.sets.length - 1].weight, reps: ex.sets[ex.sets.length - 1].reps, done: false }],
            };
        }));
        ...
        <>
            ...
            <div className="flex items-center justify-end w-full gap-2">
                <Button onClick={() => {
                    ...
                    const savedExercises = exercises.map((ex) => {
                        const doneSets = ex.sets.map((set, i) => {
                            return {
                                set,
                                setNo: i + 1
                            }
    ...
    function PlusButton({ exerciseIndex, addSet }: { exerciseIndex: number, addSet: (exIdx: number) => void }) {
        return (
            <Button onClick={e => {
                e.stopPropagation();
